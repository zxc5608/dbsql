...
SELECT 결과를 이용하여 테이블생성하기
CREATE TABLE AS==>CTAS
1.NOTNULL을 제외한 나머지제약조건을 복사하진 않는다
2.개발시
 테스트데이터를 만들어놓고 놓고실험을 해보고싶을떄
 개발자 수준의 데이터백업
...
테이블 변경
1.새로운 컬럼을 추가
2.기존에 존재하는 컬럼의 변경(이름,데이터타입)
 * * 데이터 타입의 경우 이미데이터가 존재하면 수정이 불가능하다고 보면 됨
...
DESC emp_test;
emp_test 테이블에hp컬럼을 VARCHAR2(15)로 추가
...
(***데이터 타입을 바꾸는 것은 데이터가 없을때는 상관없지만
데이터가 있는 경우는 사이즈를 늘리는것을 제외하고는 불가능.
...
CREATE TABLE emp_test(
 ...
 1.dept _test 테이블의 deptno컬럼에 PRIMARY KEY 제약조건 추가
 ATLER TABLE
 2.emp_test 테이블의 empno컬럼에  프라이머리 키 제약조건 추가
